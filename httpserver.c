...
int add_fileLock(FileLockStruct *array, char *URI, int size) {
  ...
  if (pos != -1) {
    ...
    array[pos].count++;
    array[pos].URI =
        calloc(strlen(URI) + 1, sizeof(char)); // +1 to include termination byte
...
ssize_t parseRequest(char *requestBuffer, Request request, int *status_code) {
  const char *re = "^([a-zA-Z]{0,8}) (/[a-zA-Z0-9.-]{1,63}) "
                   "(HTTP/[0-9]\\.[0-9])\r\n(.|\n)*$";
  regex_t regex;
  regmatch_t pmatch[5];
...
void shiftBuffer(char *buf, ssize_t max_size, ssize_t shift) {
  // Shifts a buffer by shift amount and fill up the buffer with data from
  // socket
...
void *server_thread(void *arg) {
  ...
  while (1) {
    ...
    if (request_bytes ==
        -1) { // If parsing fails, produce a response and nothing else
